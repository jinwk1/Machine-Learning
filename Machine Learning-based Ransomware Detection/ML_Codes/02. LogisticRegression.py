...
for C in [0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000]:
    for penalty in ["l2"]:
        logreg_alpha = LogisticRegression(C=C, penalty=penalty, solver='liblinear')
        logreg_alpha.fit(X_train, y_train)
        training_score = logreg_alpha.score(X_train, y_train)
        valid_score = logreg_alpha.score(X_valid, y_valid)
